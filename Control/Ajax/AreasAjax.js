...
function listar()
{
	tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
	{
	    ...
	    "ajax"://metodo ajax
				{
					url: '../Control/AreasControl.php?op=listar',//pagina que realiza la operación
...
function guardar(e)
{
	...
	$.ajax({
		url: "../Control/AreasControl.php?op=guardar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(id)
{
	$.post("../Control/AreasControl.php?op=mostrar",{id : id}, function(data, status)
	{
...
function borrar(id)
{
	bootbox.confirm("¿Está Seguro de borrar el empleado?", function(result){
		if(result)
        {
        	$.post("../Control/AreasControl.php?op=borrar", {id : id}, function(e)
        	{
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
